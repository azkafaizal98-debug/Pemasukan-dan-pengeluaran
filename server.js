...
// Export entries as CSV or XLSX. Optional query: month=YYYY-MM to filter by month
...
// Import entries from uploaded Excel/CSV file (multipart/form-data, field name 'file')
app.post('/api/import', upload.single('file'), async (req, res) => {
  ...
  // Expect columns: amount, type, category, date, note (id ignored)
  const db = await readDB();
  let added = 0;
  for (const row of data) {
    ...
    const entry = { id: uuidv4(), amount, type, category: row.category || row.Category || 'Umum', date: row.date ? new Date(row.date).toISOString() : new Date().toISOString(), note: row.note || row.Note || '' };
    db.entries.unshift(entry);
    added++;
  }
  await writeDB(db);
...
// New APIs for additional features
...
app.post('/api/recurring', async (req, res) => {
  ...
  if (typeof amount !== 'number' || !['income', 'expense'].includes(type) || !['daily', 'weekly', 'monthly', 'yearly'].includes(frequency)) {
